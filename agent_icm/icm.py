...
class ICM(nn.Module):
    def __init__(self, input_dims, n_actions = 3, alpha=0.1, betta=0.2):
    ...
    def calc_loss(self, states, new_states, actions):
        ...
        instrinct_reward = self.alpha * 0.5 *((phi_hat_new-phi_new).pow(2)).mean(dim=1)

        return instrinct_reward, L_I, L_F